...
dirac_points = [[pi/3,pi/3], [pi/3,-pi/3], 
                [-pi/3,pi/3], 
                [-pi/3,-pi/3]]
...
kvals1 = bz1.kvals
kvals2 = bz2.kvals
matches1 = findall(k -> any(dp -> isapprox(k, dp; atol = 1e-02), dirac_points), eachcol(kvals1))
matches2 = findall(k -> any(dp -> isapprox(k, dp; atol = 1e-02), dirac_points), eachcol(kvals2))
println("Dirac point matches in bz1 at indices: ", matches1)
println()
println("Dirac point matches in bz2 at indices: ", matches2)
